...
/**
 * Extend the basic Bitcoin Client to support promises
 * Based upon the bitcoin-promise NPM module, but extensible so we can add Omni methods.
 */
class BitcoinPromisesClient extends BitcoinClient {

  constructor(opts) {
    super(opts);
    // The below code copied from bitcoin-promise and can probably be optimized
    // now that we are using it in a subclass/constructor.
    // Find all methods that are actually RPC calls
    var methods = Object.getOwnPropertyNames(BitcoinClient.prototype).filter(function(p) {
    ...
    for (var i = 0 ; i<methods.length ; i++ ) {
      ...
      (function(protoFn) {
        BitcoinClient.prototype[protoFn] = function() {
...
class OmniClient extends BitcoinPromisesClient {
  constructor(opts) {
    super(opts);
    for (var method in OmniClientCommands) {
      (function(methodName) {
...
/**
 * Omni is from an earlier version of the client and is deprecated and will be removed.
